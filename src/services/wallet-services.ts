...
export class WalletServices {
  static async create(accountId: string, wallet: CreationWallet) {
    ...
    return await getPrismaClient().wallet.create({ data: WalletMapper.create(accountId, { ...wallet, isArchived: false }) });
  }
  ...
  static async getAllArchived(accountId: string, query: ListFilters & NameFilter & WalletFilter) {
    const { page = 1, pageSize = 10, name, isActive, walletType } = query;

    const where = { accountId, name: { contains: name }, ...filterIfNotNull("isActive", isActive), ...filterIfNotNull("type", walletType), isArchived: true };
